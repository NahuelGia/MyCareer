...
export const StatisticsModal = ({
	...
	const creditsData = [
		...
		{
			name: "Créditos restantes",
			value: totalCredits - creditsEarned,
			fill: "#A0AEC0",
		},
	];
	...
	const getPromedioByPeriod = () => {
		...
		const completedSubjects = subjects.filter(
			(subject) =>
				subject.data.status === "Completada" &&
		...
		const periodAverages = Object.entries(subjectsByPeriod).map(
			([period, periodSubjects]) => {
				const average =
					periodSubjects.reduce((sum: number, subject: Subject) => {
	...
	const calculateDetailedSubjectsPerSemester = () => {
		...
		const subjectsByPeriod: Record<string, Subject[]> = {};
		const completedSubjects = subjects.filter(
			(subject) => subject.data.status === "Completada" && subject.data.periodo
		);
		...
		const periodsData = Object.entries(subjectsByPeriod).map(
			([period, periodSubjects]) => ({
				period,
		...
		const averagePerSemester =
			totalPeriods > 0 ? totalSubjects / totalPeriods : 0;
	...
	const calculateWeeklyHoursByPeriod = () => {
		...
		const periodHoursData = Object.entries(subjectsByPeriod).map(
			([period, periodSubjects]) => {
			...
			>
				...
				<Grid templateColumns={{base: "1fr", md: "repeat(2, 1fr)"}} gap={6}>
					...
					<GridItem>
						...
						<Text textAlign="center" mb={4}>
							{approved} / {approved + remaining} materias
						</Text>
						<ResponsiveContainer width="100%" height={200}>
							<PieChart>
								<Pie
						...
						<Center>
							<VStack>
								<Text
									...
									fontWeight="bold"
									color={
										subjectsPerSemesterData.estimatedTerms > 5
											? "red.500"
											: "green.500"
									}
								>
								...
								<Text fontSize="sm" color="gray.600" textAlign="center">
									Basado en un promedio de{" "}
									{subjectsPerSemesterData.averagePerSemester.toFixed(1)}{" "}
					...
					<GridItem>
						...
						<ResponsiveContainer width="100%" height={200}>
							<BarChart data={subjectsPerSemesterData.periodsData}>
								...
								/>
								<ReferenceLine
									y={subjectsPerSemesterData.averagePerSemester}
									stroke="red"
									strokeDasharray="3 3"
								>
					...
					<GridItem>
						<Heading size="md" mb={2} textAlign="center">
							Horas de estudio semanales por periodo
						</Heading>
						<ResponsiveContainer width="100%" height={200}>
							<LineChart data={weeklyHoursData}>
								...
								/>
								<Tooltip
									formatter={(value) => [`${value} horas`, "Horas semanales"]}
								/>
								<Legend />
								<Line
									type="monotone"
								...
								<ReferenceLine
									y={
										weeklyHoursData.reduce((sum, item) => sum + item.hours, 0) /
										weeklyHoursData.length
									}
									stroke="red"
									strokeDasharray="3 3"
								>
