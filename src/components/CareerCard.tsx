...
export const CareerCard = ({ carrera, careerProgress }: CareerCardProps) => {
  ...
  const completedSubjects = careerProgress?.data.materias.filter((m: Materia) => m.data.status === "Completada").length || 0;
      ...
      <Link to={`/${carrera.id}`} style={{ width: '100%' }}>
        <VStack align="start" gap={2}>
          <HStack width="full" justify="space-between">
            <Text fontWeight="medium">{carrera.nombre}</Text>
            {hasProgress && (
              <HStack>
                <Icon as={RiTimeLine} color="green.500" />
                <Text fontSize="sm" color="green.600">En progreso</Text>
              </HStack>
            )}
          </HStack>
          {hasProgress && (
